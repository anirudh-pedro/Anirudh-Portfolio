...
const ContactInfo = () => {
  ...
  return (
    <div className="professional-card rounded-2xl p-6 sm:p-8 shadow-xl shadow-black/30">
      ...
      <div className="space-y-5">
        {contactDetails.map(item => (
          <div key={item.id} className="flex items-start">
            <div className="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center mr-4 border border-slate-300/70">
              {item.icon}
            </div>
            <div>
              ...
              {item.link ? (
                <a 
                  href={item.link} 
                  className="text-slate-800 hover:text-slate-900 transition-colors text-base"
                >
                  {item.value}
                </a>
              ) : (
      ...
      <div className="mt-8 pt-8 border-t border-slate-300/15">
