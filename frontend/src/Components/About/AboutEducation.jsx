  ...
  return (
    ...
    >
      <h3 className="text-xl sm:text-2xl font-bold mb-6 bg-gradient-to-r from-blue-400 via-purple-500 to-indigo-600 bg-clip-text text-transparent flex items-center">
        <span className="mr-3 p-2 rounded-lg bg-gradient-to-br from-indigo-500/20 to-purple-500/20">
      ...
      <div className="space-y-6">
        {education.map((item, index) => (
          <motion.div 
            key={index} 
            className="relative pl-6 border-l-2 border-blue-400/30 hover:border-blue-400/50 transition-colors"
            variants={itemVariants}
          >
            <div className="absolute -left-1.5 mt-1.5 h-3 w-3 rounded-full bg-gradient-to-r from-blue-400 to-purple-500 shadow-lg shadow-blue-500/50"></div>
            
            <div className="mb-1 flex flex-wrap items-center">
              <h4 className="text-base sm:text-lg font-semibold text-white mr-2">{item.degree}</h4>
              <span className="text-sm sm:text-base bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent">in {item.field}</span>
            </div>
